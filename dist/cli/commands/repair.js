...
import { clearModeState, getActiveModes, MODE_CONFIGS } from '../../hooks/mode-registry/index.js';
import { createLogger } from '../../lib/unified-logger.js';
import * as readline from 'readline';
const logger = createLogger('cli:repair');
async function fixStatePollution(root, dryRun) {
...
async function fixOrphanAgents(root, dryRun) {
...
async function validateStateFiles(root) {
...
async function interactiveRepair(root) {
    ...
    if (choice === '1' || choice === 'all')
        await fixStatePollution(root, dryRun);
    if (choice === '2' || choice === 'all')
        await fixOrphanAgents(root, dryRun);
    if (choice === '3' || choice === 'all')
        await validateStateFiles(root);
}
export function createRepairCommand() {
    const cmd = new Command('repair')
        ...
        if (options.fixStatePollution)
            await fixStatePollution(root, dryRun);
        if (options.fixOrphanAgents)
            await fixOrphanAgents(root, dryRun);

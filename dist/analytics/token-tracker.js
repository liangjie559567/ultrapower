...
const SESSION_INDEX_FILE = path.join(homedir(), ".omc", "state", "token-tracking-index.json");
...
export class TokenTracker {
    currentSessionId;
    sessionStats;
    constructor(sessionId, skipRestore) {
        ...
        if (!skipRestore) {
            const existing = readState("ultrawork", StateLocation.GLOBAL);
            if (existing.exists &&
                existing.data &&
    ...
    async loadSessionStats(sessionId) {
        ...
        const result = readState("ultrawork", StateLocation.GLOBAL);
    ...
    async rebuildStatsFromLog(sessionId) {
        try {
            ...
            if (index?.sessions[sessionId]) {
                ...
                const endOffset = Math.min(allLines.length, startOffset + sessionInfo.count + 10);

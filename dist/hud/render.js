...
export async function render(context, config) {
    ...
    const ctxWarning = renderContextLimitWarning(context.contextPercent, config.contextLimitWarning.threshold, config.contextLimitWarning.autoCompact, context.suppressCompactWarning);
    if (ctxWarning)
    ...
    if (context.smartSuggestions && context.smartSuggestions.length > 0) {
        ...
        if (suggestionsLine)
            detailLines.push(suggestionsLine);
    }

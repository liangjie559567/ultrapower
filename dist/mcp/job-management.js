 ...
 */
function _jobStatusToWorkerState(job) {
    return {
            ...
            if (worker) {
                if (worker.status === 'completed' || worker.status === 'failed' || worker.status === 'timeout') {
                    if (worker.status === 'completed') {
                        const slug = worker.metadata?.slug || '';
    ...
    if (adapter) {
        ...
        if (worker) {
            ...
            try {
                ...
                return textResult(`Failed to kill process ${worker.pid}: ${err.message}`, true);
            }
        }
    }
    const jobDir = getJobWorkingDir(provider, jobId);
    ...
    if (adapter) {
        ...
        const lines = limited.map((worker) => {
            ...
            if (worker.completedAt)
                parts.push(`  Completed: ${worker.completedAt}`);
            if (worker.error)
                parts.push(`  Error: ${worker.error}`);
            if (worker.pid)

...
export function enforceModel(agentInput) {
    ...
    // Extract agent type (strip ultrapower: prefix if present)
    const agentType = agentInput.subagent_type.replace(/^ultrapower:/, '');
 ...
 */
function resolveTierModel(agentType) {
...
export function processPreToolUse(toolName, toolInput) {
    ...
    try {
        ...
        const agentType = toolInput.subagent_type.replace(/^ultrapower:/, '');
        const tierModel = resolveTierModel(agentType);
        if (tierModel !== null) {
            const modifiedInput = { ...toolInput, model: tierModel };
            let warning;
            if (process.env.OMC_DEBUG === 'true') {
 ...
 *
 * @param agentType - The agent type (with or without ultrapower: prefix)
...
export function getModelForAgent(agentType) {
    const normalizedType = agentType.replace(/^ultrapower:/, '');
    const agentDefs = getAgentDefinitions();

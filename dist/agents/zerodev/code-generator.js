...
export function matchTemplate(requirement) {
    if (!requirement?.trim()) {
...
export function generateCode(template, vars) {
    if (!template?.trim()) {
...
async function validateCodeWithLSP(code) {
    const tempDir = join(tmpdir(), '.omc', 'zerodev', 'temp');
    mkdirSync(tempDir, { recursive: true });
    const tempFile = join(tempDir, `temp-${Date.now()}.ts`);
    try {
        writeFileSync(tempFile, code);
        const { mcp__plugin_ultrapower_t__lsp_diagnostics } = globalThis;
...
function checkQualityLegacy(code) {
    let score = 50;
...
export async function checkQuality(code) {
    try {

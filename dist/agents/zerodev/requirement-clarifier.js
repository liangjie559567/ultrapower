...
export function detectPlatform(input, memory) {
    if (!input?.trim()) {
    ...
    // 优先从项目记忆推断
    if (memory?.techStack) {
        if (memory.techStack.some(t => t.includes('React Native') || t.includes('Flutter')))
            return 'mobile';
        if (memory.techStack.some(t => t.includes('Electron')))
            return 'desktop';
    }
    if (lower.includes('api') || lower.includes('接口'))
        return 'api';
    if (lower.includes('网站') || lower.includes('web'))
        return 'web';
    if (lower.includes('手机') || lower.includes('mobile'))
        return 'mobile';
    if (lower.includes('移动应用') || lower.includes('app'))
        return 'mobile';
    if (lower.includes('插件') || lower.includes('extension'))
        return 'plugin';
    if (lower.includes('桌面') || lower.includes('desktop'))
        return 'desktop';
    // 默认推断为 web
    if (lower.includes('应用') || lower.includes('系统') || lower.includes('平台'))
        return 'web';
    return 'unknown';
}
...
//# sourceMappingURL=requirement-clarifier.js.map
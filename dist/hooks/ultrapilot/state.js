...
export function readUltrapilotState(directory, sessionId) {
    ...
    if (sessionId) {
        ...
        if (existsSync(sessionFile)) {
            try {
                ...
                const result = safeJsonParse(content, sessionFile);
                if (result.success) {
                    return result.data;
                }
            }
    ...
    try {
        ...
        const result = safeJsonParse(content, stateFile);
 ...
 */
export async function writeUltrapilotState(directory, state, sessionId) {
 ...
 */
export async function initUltrapilot(directory, task, subtasks, sessionId, config) {
 ...
 * Update worker state
 */
export async function updateWorkerState(directory, workerId, updates, sessionId) {
    const state = readUltrapilotState(directory, sessionId);
    if (!state)
        return false;
 ...
 * Add a new worker
 */
export async function addWorker(directory, worker, sessionId) {
    const state = readUltrapilotState(directory, sessionId);
    if (!state)
        return false;
 ...
 */
export async function completeWorker(directory, workerId, filesCreated, filesModified, sessionId) {
    const state = readUltrapilotState(directory, sessionId);
    if (!state)
        return false;
    const workerIndex = state.workers.findIndex((w) => w.id === workerId);
 ...
 */
export async function failWorker(directory, workerId, error, sessionId) {
    const state = readUltrapilotState(directory, sessionId);
    if (!state)
        return false;
    const workerIndex = state.workers.findIndex((w) => w.id === workerId);
 ...
 */
export async function completeUltrapilot(directory, sessionId) {
    const state = readUltrapilotState(directory, sessionId);
    if (!state)
        return false;
    state.active = false;
...
export function readFileOwnership(directory, sessionId) {
    ...
    try {
        ...
        const result = safeJsonParse(content, ownershipFile);
        return result.success ? result.data : null;
    }
    catch {
        return null;
 ...
 */
export async function recordConflict(directory, filePath, sessionId) {
    const state = readUltrapilotState(directory, sessionId);
    if (!state)
        return false;

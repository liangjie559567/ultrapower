 ...
 * Ralph Hook
 *
 * Self-referential work loop that continues until cancelled via /ultrapower:cancel.
 ...
 */
function _getStateFilePath(directory, sessionId) {
    if (sessionId) {
 ...
 */
function _ensureStateDir(directory, sessionId) {
    if (sessionId) {
...
export function readRalphState(directory, sessionId) {
    const adapter = createStateAdapter('ralph', directory);

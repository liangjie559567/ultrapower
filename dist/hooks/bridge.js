...
async function readTeamStagedState(directory, sessionId) {
...
function _readTeamStagedStateSync(directory, sessionId) {
    const stateDir = join(directory, ".omc", "state");
    const statePaths = sessionId
        ? [
            join(stateDir, "sessions", sessionId, "team-state.json"),
 ...
 */
async function processPreToolUse(input) {
    const directory = resolveToWorktreeRoot(input.directory);
...
async function processPostToolUse(input) {
    ...
    // (e.g., ralplan consensus handoff into Skill("ultrapower:ralph")).

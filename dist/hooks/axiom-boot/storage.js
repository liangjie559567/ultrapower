...
export function getAxiomDir(workingDirectory) {
    return join(workingDirectory, AXIOM_DIR);
}
export function isAxiomEnabled(workingDirectory) {
    return existsSync(getAxiomDir(workingDirectory));
}
export function getMemoryFilePaths(workingDirectory) {
...
function truncateAxiomFile(content, filePath) {
    if (content.length <= MAX_AXIOM_FILE_CHARS)
        return content;
    return content.slice(0, MAX_AXIOM_FILE_CHARS) + `\n\n[截断，完整内容见: ${filePath}]`;
}
export function readActiveContext(workingDirectory) {
    const paths = getMemoryFilePaths(workingDirectory);
    if (!existsSync(paths.activeContext))
        return null;
    const raw = readFileSync(paths.activeContext, 'utf-8');
    return truncateAxiomFile(raw, paths.activeContext);
}
export function readActiveContextRaw(workingDirectory) {
    const paths = getMemoryFilePaths(workingDirectory);
    if (!existsSync(paths.activeContext))
        return null;
    return readFileSync(paths.activeContext, 'utf-8');
}
export function parseAxiomState(activeContextContent) {
...
`;
export function ensureConstitution(workingDirectory) {
    const axiomDir = getAxiomDir(workingDirectory);
    if (!existsSync(axiomDir))
        return; // Axiom 未初始化，不创建

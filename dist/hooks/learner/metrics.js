...
export class WorkflowMetrics {
    ...
    startTracking(workflow) {
        this.timers.set(workflow, Date.now());
    }
    async endTracking(workflow, options = {}) {
        let durationMin;
    ...
    async getInsights(workflow) {
        ...
        let suggestion = '';
        if (avgDuration > 30)
            suggestion = `平均耗时 ${avgDuration.toFixed(0)} min 偏高，考虑拆分工作流`;
        else if (successRate < 0.8)
            suggestion = `成功率 ${(successRate * 100).toFixed(0)}% 低于 80%，建议检查常见失败原因`;
        return {
            workflow,
    ...
    async appendRun(run) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (inSection && line.startsWith('## ') && !line.includes(marker))
                break;
            if (inSection && line.includes('| Date |')) {
                inTable = true;
                continue;
            }

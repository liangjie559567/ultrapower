...
export class PatternDetector {
    ...
    async detectAndUpdate(diffText) {
        const matches = this.detectFromDiff(diffText);
        const current = await this.loadPatterns();
        const result = { newPatterns: [], promoted: [], matches: matches.map(m => m.patternName) };
        const patternFiles = new Map();
        for (const m of matches) {
            if (!patternFiles.has(m.patternName))
                patternFiles.set(m.patternName, new Set());
    ...
    async writePatterns(patterns) {
        ...
        const lines = [
            ...
            '|---|---|---|---|---|---|',
            ...patterns.map(p => `| ${p.id} | ${p.name} | ${p.category} | ${p.occurrences} | ${p.confidence.toFixed(2)} | ${p.status} |`),
            '',
        ];

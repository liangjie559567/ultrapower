...
export async function reflectOnSessionEnd(input) {
    const work = async () => {
        try {
            ...
            if (input.modesUsed && input.modesUsed.length > 0) {
                ...
                for (const mode of input.modesUsed) {
                    await learningQueue.addItem('session_mode', mode, 'P3', `mode used in session ${sessionName}`);
                    queuedItems++;
                }
            }
...
//# sourceMappingURL=session-reflector.js.map
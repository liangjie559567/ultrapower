...
If **Codex MCP unavailable** → use `Task(subagent_type="ultrapower:executor", model="sonnet")` for backend tasks.

If **Gemini MCP unavailable** → use `Task(subagent_type="ultrapower:designer", model="sonnet")` for frontend tasks.
...
Stop active CCG work: say `cancelomc` or run `/ultrapower:cancel`.

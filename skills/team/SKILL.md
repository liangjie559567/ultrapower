...
1. **TaskList 轮询** -- 定期调用 `TaskList` 检查整体进度：
   ```
...
2. **关闭队友** -- 向每个活跃队友发送 `shutdown_request`：
   ...
   ```
1. **等待响应** -- 每个队友以 `shutdown_response(approve: true)` 响应并终止
2. **删除 team** -- 调用 `TeamDelete` 清理：
   ```json
   { "team_name": "fix-ts-errors" }
   ```
   响应：
   ...
   ```
1. **清理 OMC 状态** -- 删除 `.omc/state/team-state.json`
2. **报告摘要** -- 向用户展示结果
...
1. WORK: Execute the task using your tools (Read, Write, Edit, Bash).
   Do NOT spawn sub-agents. Do NOT delegate. Work directly.

1. COMPLETE: When done, mark the task completed:
   {"taskId": "ID", "status": "completed"}

1. REPORT: Notify the lead via SendMessage:
   {"type": "message", "recipient": "team-lead", "content": "Completed task #ID: <summary of what was done>", "summary": "Task #ID complete"}

1. NEXT: Check TaskList for more assigned tasks. If you have more pending tasks, go to step 1.
...
1. SHUTDOWN: When you receive a shutdown_request, respond with:
...
1. 通过 MCP 工具清理 OMC 状态：
   ...
   ```
1. 或运行 `/ultrapower:cancel`，它自动处理所有清理。
...
1. **Worker 隔离**：将 worktree 路径作为 worker `BridgeConfig` 中的 `workingDirectory` 传递。Worker 仅在其自己的 worktree 中运行。

1. **合并协调**：Worker 完成任务后，使用 `checkMergeConflicts()` 验证分支可以干净合并，然后使用 `mergeWorkerBranch()` 以 `--no-ff` 合并以保留清晰历史。

1. **Team 清理**：Team 关闭时，调用 `cleanupTeamWorktrees(teamName, repoRoot)` 删除所有 worktree 及其分支。
...
1. **无原子认领** -- 与 SQLite swarm 不同，`TaskUpdate` 没有事务保证。两个队友可能竞争认领同一任务。**缓解措施：** Lead 应在生成队友前通过 `TaskUpdate(taskId, owner)` 预分配所有者。队友只应处理分配给自己的任务。

1. **任务 ID 是字符串** -- ID 是自增字符串（"1"、"2"、"3"），而非整数。始终向 `taskId` 字段传递字符串值。

1. **TeamDelete 需要空 team** -- 调用 `TeamDelete` 前所有队友必须已关闭。Lead（唯一剩余成员）不在此检查范围内。

1. **消息自动传递** -- 队友消息作为新对话轮次到达 lead。入站消息无需轮询或检查 inbox。但如果 lead 正在处理中（mid-turn），消息会排队并在轮次结束时传递。

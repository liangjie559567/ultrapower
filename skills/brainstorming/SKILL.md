...
1. explore 扫描内容：项目结构、相关文件、近期5个 commit、现有约定

1. 将探索结果写入 notepad（`notepad_write_working`，key: codebase_summary）
...
2. **提出澄清问题（5W1H）** —— 系统化提问，覆盖所有维度
3. **搜索外部资料** —— 调用 sciomc 和 external-context 搜索最佳实践
4. **提出 2-3 种方案** —— 附带权衡分析、参考资料和推荐意见
...
digraph brainstorming {
    ...
    "Ask clarifying questions (5W1H)" [shape=box];
    "Search external resources" [shape=box];
    ...
    "Explore project context" -> "Ask clarifying questions (5W1H)";
    "Ask clarifying questions (5W1H)" -> "Search external resources";
    "Search external resources" -> "Propose 2-3 approaches";
...
* 使用 5W1H 框架系统化提问（必须覆盖所有维度）：
  - **Why（为什么）**: 这个功能解决什么问题？用户痛点是什么？
  - **What（是什么）**: 具体要实现什么功能？核心能力是什么？
  - **Who（谁）**: 目标用户是谁？谁会使用这个功能？
  - **When（何时）**: 什么场景下触发？使用频率如何？
  - **Where（哪里）**: 在系统的哪个位置？与哪些模块交互？
  - **How（如何）**: 期望的用户体验是什么？有哪些约束条件？
...
* **完整性检查**：在进入方案探索前，确认 5W1H 所有维度都已明确

**探索方案（含外部资料搜索）：**

* **强制前置：外部资料搜索**
  - 调用 `sciomc` skill 搜索相关最佳实践、学术论文、技术文章
  - 调用 `external-context` skill 搜索相关开源项目、GitHub 仓库
  - 将搜索结果写入 notepad（`notepad_write_working`，key: external_research）

* **强制要求：至少 3 种方案**
  - 必须提出至少 3 种不同的实现方案
  - 如果只有 2 种明显方案，第 3 种可以是"混合方案"或"渐进式方案"
...
* 以推荐选项开头并解释原因（基于外部资料验证）

* **可选：专家评审**（复杂项目推荐）
  - 对于复杂或高风险项目，调用 `ax-review` 进行 5 专家并行评审
  - 评审维度：UX、产品、领域、技术、安全

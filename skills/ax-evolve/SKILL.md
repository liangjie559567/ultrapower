...
- **手动**：用户输入 `/ax-evolve`
- **手动归档**：用户输入 `/ax-evolve --archive-queue`（仅触发队列归档，不执行其他步骤）
...
归档检查：当 done 条目数 > 10 时，自动将较旧的条目移入
`.omc/axiom/evolution/learning_queue_archive.md`，主文件保留最近 10 条 done。

若用 `--archive-queue` 参数调用：
```
调用 EvolutionOrchestrator.archiveQueue()，输出归档结果：
[归档] learning_queue: 已移动 N 条 done 条目至 learning_queue_archive.md
```

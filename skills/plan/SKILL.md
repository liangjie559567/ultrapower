...
1. **Architect** reviews architectural soundness（优先使用 `ask_codex` 的 `architect` 角色）
2. **Critic** evaluates against quality standards（优先使用 `ask_codex` 的 `critic` 角色）
3. **重新审查循环**（最多 5 次迭代）：如果 Critic 拒绝，执行此闭合循环：

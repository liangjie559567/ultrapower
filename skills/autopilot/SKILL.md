...
- Cancel with `/ultrapower:cancel` at any time; progress is preserved for resume
</Execution_Policy>
...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   ...
   - Run `/ultrapower:cancel` for clean exit
</Steps>
...
If autopilot was cancelled or failed, run `/ultrapower:autopilot` again to resume from where it stopped.

...
async function main() {
  try {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      if (pkg?.name === '@liangjie559567/ultrapower' && pkg?.version) {
...
- "Upgrade now" (Recommended): Run \`npm install -g @liangjie559567/ultrapower@latest\` via Bash, then run \`omc install --force --skip-claude-check --refresh-hooks\` to reconcile hooks and CLAUDE.md
...
A new version of ultrapower is available: v${updateInfo.latestVersion} (current: ${updateInfo.currentVersion})

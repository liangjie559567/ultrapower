...
1. **team-pipeline** - 团队协作流水线
   - 文件: index.ts, state.ts
   - 预计用例: 6 个

1. **persistent-mode** - 持久模式管理
...
1. **mode-registry** - 模式注册
2. **observability** - 可观测性
3. **subagent-tracker** - 子 Agent 追踪
4. **nexus** - 事件同步
5. **beads-context** - 上下文管理
6. **keyword-detector** - 关键词检测
7. **empty-message-sanitizer** - 消息清理
8. **todo-continuation** - 任务延续
...
1. **agent-usage-reminder** - Agent 使用提醒
2. **auto-slash-command** - 自动斜杠命令
3. **background-notification** - 后台通知
4. **comment-checker** - 注释检查
5. **directory-readme-injector** - README 注入
6. **plugin-patterns** - 插件模式
7. **pre-compact** - 预压缩
8. **preemptive-compaction** - 抢占式压缩
9. **thinking-block-validator** - 思考块验证
10. **think-mode** - 思考模式

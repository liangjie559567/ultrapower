...
2. parser.test.ts 初始 1 个失败：
   - 修复: 调整无效 frontmatter 测试预期（parser 成功提取结构，仅返回 body）

3. matcher.test.ts 初始 1 个失败：
...
2. 最后 1 个测试失败："finds messages where thinking is not first"
...
2. 修复测试 2："lspCodeActionsTool returns code actions when found"

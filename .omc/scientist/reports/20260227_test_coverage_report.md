...
1. **hooks.test.ts 覆盖范围不明确**：该文件 282 个断言覆盖多个 hooks 子模块，但通过顶层 import 而非子模块直接测试，实际覆盖的代码路径需运行覆盖率工具确认。

1. **集成测试无法量化**：`tests/` 目录的 E2E 测试依赖 Claude CLI，无法在本次静态分析中评估其有效性。

1. **测试质量主观判断**：断言数量不等于测试质量，部分低断言数文件可能测试了高价值路径。

1. **动态行为未覆盖**：team/ 模块的 heartbeat、worker-restart 等涉及进程管理的功能，静态分析无法判断是否有运行时测试。

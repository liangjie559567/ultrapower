...
1. **目录创建**: 每个模块都实现 `ensureStateDir()` 函数
   - `mkdirSync` + `recursive: true`
   - 会话目录 vs 遗留目录的分支逻辑

1. **读取逻辑**: 重复的 `readXxxState()` 函数
   - `existsSync` 检查
   - `readFileSync` + `JSON.parse`
   - 会话 ID 验证（`state.session_id !== sessionId`）
   - 错误处理（返回 `null`）

1. **写入逻辑**: 重复的 `writeXxxState()` 函数
   - `ensureStateDir` 调用

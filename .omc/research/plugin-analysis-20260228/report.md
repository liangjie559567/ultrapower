...
3. **实现 syncMarketplace()** — 在 npm publish 后自动更新 marketplace.json 版本号并推送到 GitHub，打通自动更新链路
4. **评估范围版本** — 测试 marketplace.json 中使用 `"^5.x.x"` 是否被 Claude Code 正确解析
5. **确认 SessionStart matcher 语义** — 验证非工具事件的 matcher 行为
6. **确认 async 字段** — 验证 `"async": false` 是否为官方支持的字段

### 长期优化（P2）

7. **添加 lspServers 声明** — 通过 `.lsp.json` 正式声明 LSP 能力
8. **添加 outputStyles** — 利用官方输出样式自定义能力
9. **改进 MCP 服务器命名** — 使用更具描述性的名称避免冲突
10. **添加团队分发文档** — 提供 `extraKnownMarketplaces` 配置模板

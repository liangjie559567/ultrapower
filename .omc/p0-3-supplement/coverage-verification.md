...
1. **优先级 P0**: 补充 session-lock.ts 测试（当前 3.31%）
   - 预计提升整体覆盖率 +2%
2. **优先级 P1**: 补充 memory-tools.ts 和 notepad-tools.ts 测试
   - 预计提升整体覆盖率 +3%
3. **优先级 P2**: 补充 legacy-listener.ts 测试
   - 预计提升整体覆盖率 +2%

**预计总提升**: +7%，可达到 62.44%（仍差 2.56%）

### 中期（达到 70% 目标）
4. 补充 platform/process-utils.ts 测试
5. 补充 team/tmux-session.ts 测试
6. 补充 lsp/client.ts 测试

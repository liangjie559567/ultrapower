...
- 添加时间: 2026-03-05
- 处理时间: 2026-03-05
- 内容: 复杂项目深度研究使用团队模式分阶段执行：探索阶段（1-2 agents 并行）→ 专业分析阶段（5-6 agents 并行）→ 整合阶段（1 agent）→ 报告生成阶段（1-2 agents）。关键：使用 TaskUpdate addBlockedBy 管理依赖，确保阶段性解锁。优势：执行时间降低 80%（40min → 8min），分析深度提升，质量提升 80%。适用场景：多维度分析、代码库审计、架构评估。
- 元数据: session=2026-03-05, agents=10, phases=4, time_saved=80%
- 知识产出: k-077

### LQ-043: Agent 模型路由成本优化策略
- 优先级: P2
- 来源类型: session
- 状态: done
- 添加时间: 2026-03-05
- 处理时间: 2026-03-05
- 内容: 按任务复杂度分配模型降低成本：Haiku（快速扫描、简单查找）、Sonnet（标准实现、中等分析）、Opus（架构设计、复杂推理）。ultrapower 深度研究案例：explorer(haiku)、5个分析agents(sonnet)、architect+strategist(opus)。成本降低约 40%，性能无损。关键：在 Task() 调用时传入 model 参数。
- 元数据: session=2026-03-05, cost_reduction=40%, agents=10
- 知识产出: k-078
...
- 添加时间: 2026-03-05
- 处理时间: 2026-03-05
- 内容: 使用 TaskUpdate addBlockedBy 管理任务依赖，实现阶段性解锁。模式：创建所有任务 → 设置依赖关系 → 启动第一阶段 → 监控完成 → 解锁下一阶段。优势：避免资源浪费（agents 不会提前启动），确保执行顺序，清晰的进度可视化。注意：TaskUpdate 后立即 SendMessage 通知 agent，避免 agents 报告"等待任务完成"。
- 元数据: session=2026-03-05, pattern=phased-unlock, tools=TaskUpdate+SendMessage
- 知识产出: k-079

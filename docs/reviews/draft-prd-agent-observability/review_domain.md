...
1. **P95 实现未定义**：SQLite 无原生百分位函数，PRD 需明确实现策略（SQL 模拟 vs 应用层计算），并指定查询窗口大小以保证性能。

1. **token 存储粒度不足**：必须存储四维 token 数据（input/output/cache_write/cache_read），仅存总成本会丢失 cache 优化分析能力。

1. **缺少 `parent_run_id`**：没有 span 关联，tool_calls 表是孤立数据，无法重建 agent 执行链，可观测性价值大幅降低。

...
1. **超长查询字符串**：无长度限制的 `--filter` 参数可触发 MiniSearch 在大数据集下的高 CPU 消耗。当前 62 条规模下影响有限，但 PRD 未定义最大查询长度。
...
1. **D-C02（HIGH）**：`stop_hook_active` 守卫的描述与实际实现机制不符，且在 SessionEnd hook 中从语义上就是无意义的——实现者若照 PRD 编写将产生无效守卫代码。

1. **D-C05（HIGH）**：MiniSearch `--filter` 参数的 RegExp 注入风险——降级路径无法捕获该异常，项目已有防护模式（k-034）但 PRD 未引用，须在 T3 的实现规格中强制引用。

1. **D-C04（HIGH）**：T4 ax-evolve 中途崩溃的数据一致性问题——缺少中间状态 `processing` 会导致知识库重复条目。

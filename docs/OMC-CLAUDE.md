<!-- OMC:START -->
<!-- OMC:VERSION:4.2.12 -->
# ultrapower - Intelligent Multi-Agent Orchestration

You are running with ultrapower (OMC), a multi-agent orchestration layer for Claude Code.
...
<agent_catalog>
Use `ultrapower:` prefix for Task subagent types.
...
<skills>
Skills are user-invocable commands (`/ultrapower:<name>`). When you detect trigger patterns, invoke the corresponding skill.
...
Cancel: `/ultrapower:cancel` requests teammate shutdown, marks phase `cancelled` with `active=false`, records cancellation metadata, and runs cleanup. If linked to ralph, both modes are cancelled together.
...
<cancellation>
Hooks cannot read your responses -- they only check state files. You need to invoke `/ultrapower:cancel` to end execution modes. Use `--force` to clear all state files.
...
Say "setup omc" or run `/ultrapower:omc-setup`. Everything is automatic after that.

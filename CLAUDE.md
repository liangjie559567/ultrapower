...
ultrapower v5.6.10 规范体系位于 `docs/standards/`，所有实现必须遵守：
...
1. **Hook 输入消毒**：所有 hook 输入经 `bridge-normalize.ts` 白名单过滤，未知字段被丢弃。

1. **SubagentStop 推断**：禁止直接读取 `input.success`，使用 `input.success !== false`。

# 开发准则体系

## 核心规范（必须遵循）

详细规范位于 `docs/dev-standards/` 目录：

| 文档 | 用途 |
| ------ | ------ |
| [dev-standards.md](docs/dev-standards/dev-standards.md) | 编码要求、语言规范、验证机制 |
| [context-collection.md](docs/dev-standards/context-collection.md) | 编码前强制检索清单（7步） |
| [sufficiency-checklist.md](docs/dev-standards/sufficiency-checklist.md) | 充分性验证清单（7项必查） |
| [workflow.md](docs/dev-standards/workflow.md) | 工作流程、工具链顺序 |
| [tools-integration.md](docs/dev-standards/tools-integration.md) | 工具选择决策树 |
| [quality-review.md](docs/dev-standards/quality-review.md) | 质量审查规范 |
| [context-summary-template.md](docs/dev-standards/context-summary-template.md) | 上下文摘要模板 |

## 强制执行点

1. **编码前**：执行 7 步检索清单 + 充分性验证
2. **编码中**：遵循项目约定、复用既有组件
3. **编码后**：质量审查 + 本地验证

...
export class TutorialEngine {
  static async checkAndRun(): Promise<void> {
  ...
  static async resume(state: TutorialState): Promise<void> {
    const remaining = ALL_SCENARIOS.filter(
...
`);
  ...
  static async runScenarios(): Promise<void> {
    ...
    for (const scenario of ALL_SCENARIOS) {
      if (state.completedScenarios.includes(scenario.id)) continue;

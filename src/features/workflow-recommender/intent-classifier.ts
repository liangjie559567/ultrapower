...
export function classifyIntent(prompt: string): IntentType {
  ...
  for (const [intent, patterns] of Object.entries(INTENT_PATTERNS)) {
    if (intent === 'feature-multiple') continue; // 已检查
    scores[intent as IntentType] = patterns.filter(p => p.test(prompt)).length;
  }

  const maxScore = Math.max(...Object.values(scores).filter((v): v is number => v !== undefined));
  return (Object.entries(scores).find(([_, score]) => score === maxScore)?.[0] as IntentType) || 'feature-single';
}

...
describe('state-manager cache', () => {
  ...
  describe('cache immutability', () => {
    it('should return independent clones - mutating returned data does NOT corrupt cache', () => {
      writeStateToDisk('autopilot', { active: true, value: 'original' });

      // First read populates the cache
      const result1 = readState('autopilot', StateLocation.LOCAL);
      ...
      const result2 = readState('autopilot', StateLocation.LOCAL);
    ...
    it('should return independent clones even on cache hit path', () => {
      writeStateToDisk('ralph', { active: true, count: 42 });

      // First read - populates cache
      const result1 = readState('ralph', StateLocation.LOCAL);
      // Second read - should be cache hit
      const result2 = readState('ralph', StateLocation.LOCAL);
  ...
  describe('read path purity (no write-on-read)', () => {
    it('should NOT write to disk or flip active=false for stale state on read', () => {
      ...
      const diskContent = JSON.parse(
        fs.readFileSync(path.join(TEST_STATE_DIR, 'ultrawork.json'), 'utf-8'),
      );
  ...
  describe('cache invalidation', () => {
    it('should invalidate cache on writeState', () => {
      writeStateToDisk('pipeline', { active: true, version: 1 });

      // Populate cache
      const r1 = readState('pipeline', StateLocation.LOCAL);
      expect((r1.data as Record<string, unknown>).version).toBe(1);

      // Write new data via writeState (which should invalidate cache)
      writeState('pipeline', { active: true, version: 2 }, StateLocation.LOCAL);

      // Next read should see the new data
      const r2 = readState('pipeline', StateLocation.LOCAL);
      expect((r2.data as Record<string, unknown>).version).toBe(2);
    });

    it('should invalidate cache on clearState', () => {
      writeStateToDisk('team', { active: true });

      // Populate cache
      readState('team', StateLocation.LOCAL);

      // Clear state
      clearState('team', StateLocation.LOCAL);

      // Next read should not find the state
      const r = readState('team', StateLocation.LOCAL);

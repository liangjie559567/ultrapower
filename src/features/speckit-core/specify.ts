...
import type { Constitution, Specification, Requirement } from './types.js';
...
function inferFeatureType(feature: string): string {
...
function generateOverview(feature: string, type: string): string {
...
function generateRequirements(feature: string, type: string): Requirement[] {
  const baseReqs: Requirement[] = [
    {
      id: 'REQ-001',
  ...
  const typeSpecific: Record<string, Requirement[]> = {
    api: [
...
function extractConstraints(constitution: Constitution): string[] {
  return constitution.principles.map(p => p.description);
}

function generateAcceptanceCriteria(feature: string, type: string, constitution: Constitution): string[] {
  const base = [
    'All tests pass',

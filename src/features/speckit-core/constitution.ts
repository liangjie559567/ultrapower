...
import type { Constitution, Principle } from './types.js';

export async function generateConstitution(projectPath: string): Promise<Constitution> {
...
function detectFramework(files: string[]): string | undefined {
  ...
  if (files.includes('vite.config.ts')) return 'Vite';
  return undefined;
}

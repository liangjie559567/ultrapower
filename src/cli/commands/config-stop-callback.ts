...
export function createConfigStopCallbackCommand(): Command {
  const cmd = new Command('config-stop-callback')
    ...
    .action(async (platform: string, options: any) => {
      ...
      if (options.profile) {
        ...
        if (options.webhook) platformConfig.webhookUrl = options.webhook;
        if (options.token) platformConfig.botToken = options.token;
        if (options.chat) platformConfig.chatId = options.chat;
        if (options.channelId) platformConfig.channelId = options.channelId;

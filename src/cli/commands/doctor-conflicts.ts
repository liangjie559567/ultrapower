...
import type { _PluginConfig } from '../../shared/types.js';
...
export function formatReport(report: ConflictReport, json: boolean): string {
  ...
  if (report.claudeMdStatus) {
    ...
    if (report.claudeMdStatus.hasMarkers) {
      ...
      lines.push(`    ${colors.gray('Run /ultrapower:omc-setup to add markers')}`);
      if (report.claudeMdStatus.hasUserContent) {
  ...
  if (report.hasConflicts) {
    ...
    lines.push(`${colors.gray('Review the issues above and run /ultrapower:omc-setup if needed')}`);
  } else {

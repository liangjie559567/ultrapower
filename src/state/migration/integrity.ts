 ...
 * 迁移前备份
 */
export function backupBeforeMigration(mode: ValidMode, directory: string): BackupInfo | null {
 ...
 * 回滚到备份
 */
export function rollbackToBackup(backupInfo: BackupInfo, mode: ValidMode, directory: string): boolean {
  const statePath = join(directory, '.omc', 'state', `${mode}-state.json`);
 ...
 * 验证状态完整性
 */
export function verifyIntegrity(mode: ValidMode, directory: string): IntegrityResult {
  const statePath = join(directory, '.omc', 'state', `${mode}-state.json`);
  const errors: string[] = [];
 ...
 * 清理旧备份
 */
export function cleanupOldBackups(mode: ValidMode, directory: string, keepCount: number = 5): number {

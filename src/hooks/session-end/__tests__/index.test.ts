...
describe('Session End Hook', () => {
  ...
  describe('getSessionStartTime', () => {
    it('returns undefined for non-existent state', async () => {
      const result = await getSessionStartTime(TEST_DIR);
      expect(result).toBeUndefined();
    });

    it('reads start time from state file', async () => {
      ...
      const result = await getSessionStartTime(TEST_DIR, 'test');
      expect(result).toBe('2026-03-03T10:00:00Z');
    });
  });

  describe('recordSessionMetrics', () => {
    it('records session metrics', async () => {
      const input = {
      ...
      const metrics = await recordSessionMetrics(TEST_DIR, input);
  ...
  describe('cleanupTransientState', () => {
    it('returns 0 for empty directory', async () => {
      const result = await cleanupTransientState(TEST_DIR);
      expect(result).toBe(0);
    });
  });

  describe('cleanupModeStates', () => {
    it('cleans active mode states', async () => {
      const stateDir = join(TEST_DIR, '.omc', 'state');
      mkdirSync(stateDir, { recursive: true });
      writeFileSync(
        join(stateDir, 'autopilot-state.json'),
      ...
      const result = await cleanupModeStates(TEST_DIR, 'test');
  ...
  describe('exportSessionSummary', () => {
    it('exports session summary', async () => {
      const metrics = {
        session_id: 'test',
      ...
      await exportSessionSummary(TEST_DIR, metrics);

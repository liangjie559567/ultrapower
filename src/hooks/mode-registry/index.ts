...
import { listSessionIds, resolveSessionStatePath, getSessionStateDir, _validateSessionId } from '../../lib/worktree-paths.js';
    ...
    for (const exclusiveMode of EXCLUSIVE_MODES) {
      if (exclusiveMode !== mode && isModeActiveInAnySession(exclusiveMode, cwd)) {
        ...
        return {
          ...
          message: `Cannot start ${MODE_CONFIGS[mode].name} while ${config.name} is active. Cancel ${config.name} first with /ultrapower:cancel.`
        };
      }
    }
  }

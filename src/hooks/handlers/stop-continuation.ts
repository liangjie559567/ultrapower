...
async function readTeamStagedState(directory: string) {
  try {
    const { readStateFile } = await import("../state/StateReader.js");
    return readStateFile("team", directory) as any;
  } catch {
    return null;
  }
}

function isTeamStateTerminal(state: any): boolean {
  const phase = state.current_phase || state.stage;
  const status = state.status;
  return phase === "complete" || phase === "failed" || phase === "cancelled" || status === "cancelled";
}

function getTeamStage(state: any): string {

 ...
 */
export function parseReflectionLog(text: string): ReflectionBlock[] {
 ...
 *   - HTML 注释 <!-- ... -->
 */
function isEmptyBlock(rawContent: string): boolean {
 ...
 * 过滤掉无效内容后判断是否还有文本。
 */
function hasSectionContent(rawContent: string, sectionPattern: RegExp): boolean {
...
function isValidContentLine(line: string): boolean {
  ...
  // "(无)" 占位符（含可能的列表前缀，如 "- (无)"）
  if (trimmed === '(无)') return false;
  if (/^-\s*\(无\)$/.test(trimmed)) return false;

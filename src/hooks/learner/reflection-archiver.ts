 ...
 * @returns        归档统计结果
 */
export async function archiveReflections(
  baseDir: string,
  ...
  try {
    ...
    const blocks: ReflectionBlock[] = parseReflectionLog(content);
    ...
    // 步骤 10：输出日志
    console.log(
      `[归档] reflection_log: 已移动 ${archivedBlocks.length} 条反思至 reflection_log_archive.md`,
    );

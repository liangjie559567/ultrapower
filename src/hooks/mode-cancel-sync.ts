 ...
 */
export function clearModeWithLinked(
  mode: string,
  directory: string,
  sessionId?: string,
 ...
 */
export function linkModes(
  mode1: ValidMode,
  mode2: ValidMode,
  directory: string,
  sessionId?: string
): boolean {
  const adapter1 = createStateAdapter<ModeState>(mode1, directory);
  const adapter2 = createStateAdapter<ModeState>(mode2, directory);
  ...
  if (!state1.linkedModes) state1.linkedModes = [];
  if (!state1.linkedModes.includes(mode2)) {
    state1.linkedModes.push(mode2);
  }

  if (!state2.linkedModes) state2.linkedModes = [];

...
describe('isOmcHook()', () => {
  ...
  it('recognises ultrapower in command path', () => {
    expect(isOmcHook('/path/to/ultrapower/hook.mjs')).toBe(true);
  });
...
describe('Hook merge during omc update', () => {
  ...
  describe('edge cases', () => {
    ...
    it('handles hook group with non-command type (should not be treated as non-OMC)', () => {
      ...
      const newOmc = [omcGroup(NEW_OMC_CMD)];
      const { merged: _merged, conflicts } = mergeEventHooks(existing, newOmc, { force: true });

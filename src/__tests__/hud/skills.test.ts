...
describe('renderSkills', () => {
  ...
  describe('namespaced skill names', () => {
    it('displays only last segment for namespaced skills (ultrapower:plan)', () => {
      const lastSkill: SkillInvocation = { name: 'ultrapower:plan', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
      expect(result).toContain('skill:plan');
      expect(result).not.toContain('ultrapower');
    });

    it('displays only last segment for namespaced skills with args', () => {
      const lastSkill: SkillInvocation = { name: 'ultrapower:autopilot', args: 'build app', timestamp: new Date() };
      const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);

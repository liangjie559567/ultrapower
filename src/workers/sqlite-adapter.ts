...
export class SqliteWorkerAdapter implements WorkerStateAdapter {
  ...
  private upsertSync(worker: WorkerState): boolean {
    ...
    try {
      ...
      `);
      stmt.run(
        worker.workerId, worker.workerType, worker.name, worker.status, worker.pid ?? null,
        worker.spawnedAt, worker.lastHeartbeatAt ?? null, worker.completedAt ?? null,
        worker.provider ?? null, worker.model ?? null, worker.agentRole ?? null,
        worker.promptFile ?? null, worker.responseFile ?? null,
        worker.teamName ?? null, worker.tmuxSession ?? null, worker.currentTaskId ?? null,
        worker.consecutiveErrors ?? 0, worker.error ?? null,
        worker.metadata ? JSON.stringify(worker.metadata) : null
      );
      return true;
    } catch {

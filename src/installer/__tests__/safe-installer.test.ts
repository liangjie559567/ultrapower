...
import { existsSync, mkdirSync, writeFileSync, _readFileSync, rmSync } from 'fs';
import { join } from 'path';
import { homedir } from 'os';
import { isOmcHook, _install, InstallOptions } from '../index.js';
...
describe('isOmcHook', () => {
  ...
  it('returns true for commands containing "ultrapower"', () => {
    expect(isOmcHook('node ~/.claude/hooks/ultrapower-hook.mjs')).toBe(true);
    expect(isOmcHook('bash $HOME/.claude/hooks/ultrapower.sh')).toBe(true);
  });

  it('returns false for commands not containing omc or ultrapower', () => {

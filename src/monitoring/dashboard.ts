...
export class Dashboard {
  ...
  private async checkRegression(type: Metric['type'], current: number): Promise<string> {
  ...
  async display(hours: number = 24): Promise<void> {
  ...
  async export(format: 'json' | 'csv', outputPath: string, type?: Metric['type']): Promise<void> {
    if (format === 'json') {
